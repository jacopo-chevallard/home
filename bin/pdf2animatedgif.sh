...
display_usage() { 
	echo -e "\nDescription:\nCreates a GIF animated image from a set of input pdf files\n"
	echo -e "\nUsage:\n$0 --input/-i=[pdf file names] --output/-o=[output GIF file name]"
	echo -e "\nExample:\n$0 --input='*.pdf' --output=test.gif \n" 
	} 
...
while [ "$1" != "" ]; do
  ...
  case $PARAM in
    ...
    "--delay")  
      ...
      delay=$VALUE
      echo "Delay between frames (in 1/100th of second)----> " ${delay}
      ;;
...
convert -delay ${delay} -loop ${loop} -density ${density} ${input} ${output}
